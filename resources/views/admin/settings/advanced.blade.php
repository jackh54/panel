...
@section('content-header')
    <h1>Advanced Settings<small>Configure advanced settings for the Panel.</small></h1>
    <ol class="breadcrumb">
                            ...
                            <div class="form-group col-md-4">
                                ...
                                <div>
                                    <select class="form-control" name="turnstile:enabled">
                                        <option value="false" @if(old('turnstile:enabled', config('turnstile.enabled')) == false || old('turnstile:enabled', config('turnstile.enabled')) == '0') selected @endif>Disabled</option>
                                        <option value="true" @if(old('turnstile:enabled', config('turnstile.enabled')) == true || old('turnstile:enabled', config('turnstile.enabled')) == '1') selected @endif>Enabled</option>
                                    </select>
                                    <p class="text-muted small">If enabled, login and password reset forms will require a Cloudflare Turnstile check.</p>
                                </div>
                            </div>
                            <div class="form-group col-md-4">
                                <label class="control-label">Site Key</label>
                                <div>
                                    <input type="text" class="form-control" name="turnstile:site_key" value="{{ old('turnstile:site_key', config('turnstile.site_key') ?: config('turnstile.website_key')) }}">
                                </div>
                            </div>
                            <div class="form-group col-md-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-muted small">Used for communication between your site and Cloudflare. Keep this secret.</p>
                                </div>
                            </div>
                        </div>
                        <div class="row">
